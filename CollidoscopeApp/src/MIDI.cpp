...
void collidoscope::MIDI::setup(const Config& config)
{
  ...
  {
    logError("Error opening MIDI input device" + error.getMessage());
    return;
  }

  if (numPorts == 0) 
  {
    logError(" no MIDI input found ");
    return;
  }
  ...
  for (unsigned int portNum = 0; portNum < numPorts; portNum++) 
  {
    ...
    {
      logError("Error opening port " + std::to_string(portNum));
    }
  }
}


void collidoscope::MIDI::getMidiMessages(std::vector<MIDIMessage> & midiMessages)
{
...
collidoscope::MIDIMessage collidoscope::MIDI::parseRtMidiMessage(std::vector<unsigned char> *rtMidiMessage)
{
  ...
  const size_t numBytes = rtMidiMessage->size();
