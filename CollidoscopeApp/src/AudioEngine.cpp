...
void AudioEngine::setup(const Config& config)
{
  ...
  for (int waveIdx = 0; waveIdx < NUM_WAVES; waveIdx++)
  {
    mCursorTriggerRingBufs[waveIdx].resize(Config::CURSOR_TRIGGER_RINGBUF_LEN);
    mWaveRecordingRingBufs[waveIdx].resize(Config::NUM_CHUNKS);
    mNoteRingBufs[waveIdx].resize(Config::NOTE_RINGBUF_LEN);
  }
  ...
  for (int chan = 0; chan < NUM_WAVES; chan++)
  {
    ...
    /* buffer recorders */
    mBufferRecorderNodes[chan] = ctx->makeNode(new BufferToWaveRecorderNode(mWaveRecordingRingBufs[chan], Config::WAVE_LEN_SECONDS));
    ...
    mPGranularNodes[chan] = ctx->makeNode(new PGranularNode(mBufferRecorderNodes[chan]->getRecorderBuffer(), mCursorTriggerRingBufs[chan], mNoteRingBufs[chan]));
...
void AudioEngine::loopOn(size_t waveIdx)
{
  ...
  mNoteRingBufs[waveIdx].write(&msg, 1);
}
...
void AudioEngine::setGrainDurationCoeff(size_t waveIdx, float coeff)
{
...
void AudioEngine::getCursorTriggers(size_t waveIdx, std::vector<CursorTriggerMsg>& cursorTriggers)
{
  cursorTriggers.clear();

  const size_t availableRead = mCursorTriggerRingBufs[waveIdx].getAvailableRead();
  cursorTriggers.assign(availableRead, {});

  const bool successfulRead = mCursorTriggerRingBufs[waveIdx].read(cursorTriggers.data(), availableRead);

  CI_ASSERT(successfulRead);
}

void AudioEngine::getWaveRecordingMessages(size_t waveIdx, std::vector<RecordWaveMsg>& waveRecordingMessages)
{
  waveRecordingMessages.clear();

  const size_t availableRead = mWaveRecordingRingBufs[waveIdx].getAvailableRead();
  waveRecordingMessages.assign(availableRead, {});

  const bool successfulRead = mWaveRecordingRingBufs[waveIdx].read(waveRecordingMessages.data(), availableRead);

  CI_ASSERT(successfulRead);
}

const ci::audio::Buffer& AudioEngine::getAudioOutputBufferRef(size_t waveIdx) const
{
